...
- (NSString *)videoImagePath:(NSString *)fileName {
    NSString *path = [[ADFileTool cacheDirectory] stringByAppendingPathComponent:kVideoPic];
    [self fileManagerWithPath:path];
...
- (NSString *)receiveImagePathWithFileKey:(NSString *)fileKey
    ...
    NSString *mainFilePath = [self createFolderPahtWithMainFolder:[ADFileTool cacheDirectory] childFolder:kMyPic];
...
- (NSString *)imagePathWithName:(NSString *)imageName {
    return [[[ADFileTool cacheDirectory] stringByAppendingPathComponent:kMyPic] stringByAppendingPathComponent:imageName];
}


// origin image path
- (NSString *)originImgPath:(ADMessageFrame *)messageF {
    return [[ADMediaManager sharedManager] receiveImagePathWithFileKey:messageF.model.message.fileKey type:@"origin"];
}

// small image path
- (NSString *)smallImgPath:(NSString *)fileKey {
    return [[ADMediaManager sharedManager] receiveImagePathWithFileKey:fileKey type:@"small"];
}

- (NSString *)delieveImagePath:(NSString *)fileKey {
    NSString *fileName = [NSString stringWithFormat:@"%@%@",fileKey,@".png"];
    NSString *mainFilePath = [self createFolderPahtWithMainFolder:[ADFileTool cacheDirectory] childFolder:kDeliver];
    return [mainFilePath stringByAppendingPathComponent:fileName];
}
