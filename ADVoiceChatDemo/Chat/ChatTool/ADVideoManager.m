...
@interface ADVideoManager () <AVCaptureFileOutputRecordingDelegate>
{
...
- (void)captureOutput:(AVCaptureFileOutput *)captureOutput didFinishRecordingToOutputFileAtURL:(NSURL *)outputFileURL fromConnections:(NSArray *)connections error:(NSError *)error {
    ...
    [self compressVideo:[mp4Url path] finished:^(NSString *path) {
        ...
        // 删除原录制的文件
        if ([ADFileTool fileExistsAtPath:outputFileURL.path]) {
            [ADFileTool removeFileAtPath:outputFileURL.path];
        }
    }];
    // 直接转大小不会改变
...
- (NSString *)videoPathForMP4:(NSString *)namePath {
    NSString *videoPath   = [[ADVideoManager shareManager] videoPathWithFileName:[[namePath lastPathComponent] stringByDeletingPathExtension]];
