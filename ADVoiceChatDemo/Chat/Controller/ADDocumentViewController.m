...
#import "ADFileTool.h"
#import "ADDocumentCell.h"
#import "ADFileScanViewController.h"

@interface ADDocumentViewController () <UITableViewDataSource,UITableViewDelegate,ADDocumentCellDelegate>
...
@implementation ADDocumentViewController
...
- (void)setupNav {
    ...
    [rightButton setTitleColor:ADRGB(0x018eb0) forState:UIControlStateNormal];
...
- (void)loadData {
    NSDirectoryEnumerator * enumer = [[NSFileManager defaultManager] enumeratorAtPath:[ADFileTool fileMainPath]];
    NSString *name;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ADDocumentCell *cell = [ADDocumentCell cellWithTableView:tableView];
    cell.delegate = self;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ADDocumentCell *cell = [tableView cellForRowAtIndexPath:indexPath];
    ADFileScanViewController *scanVC = [[ADFileScanViewController alloc] init];
...
- (void)selectBtnClicked:(id)sender {
    ...
    ADDocumentCell *cell = (ADDocumentCell *)[[button superview] superview];
    ...
    for (int row = 0; row < self.dataArr.count; row++) {
        ...
        if (curIndexPath != indexPath) {
            ADDocumentCell *cell = [self.tableView cellForRowAtIndexPath:indexPath];
    ...
    if (button.selected) {
        ...
        self.navigationItem.rightBarButtonItem.enabled = NO;
        [self.rightBtn setTitleColor:ADRGB(0x018eb0) forState:UIControlStateNormal];
    }
}
