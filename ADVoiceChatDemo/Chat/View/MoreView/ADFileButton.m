...
- (void)setMessageModel:(ADMessageModel *)messageModel {
    ...
    ADFileType type = [[dicLnk objectForKey:@"x"] intValue];
    NSString *name  = [dicLnk objectForKey:@"n"];
    self.imageV.image = [ADMessageHelper allocationImage:type];
...
- (UILabel *)nameLabel {
    if (!_nameLabel) {
        ...
        _nameLabel.textColor = ADRGB(0x262626);
...
- (UILabel *)sizeLabel {
    if (!_sizeLabel) {
        ...
        _sizeLabel.textColor = ADRGB(0x707070);
    }
    return _sizeLabel;
}
...
- (UILabel *)identLabel {
    if (!_identLabel) {
        ...
        _identLabel.textColor = ADRGB(0x707070);
