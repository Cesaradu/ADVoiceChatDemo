...
- (void)recordVideoStarted {
    ...
    [[ADVideoManager shareManager] startRecordingVideoWithFileName:self.videoName];
}
...
- (void)recordVideoFinished {
    ...
    if ((double)timeInterval <= kDurationTime) { //小于或等于规定时间
        ...
        [[ADVideoManager shareManager] stopRecordingVideo:^(NSString *path) {
            NSDictionary *userInfo = @{VideoPathKey : path};
            [self routerEventWithName:ADRouterEventVideoRecordFinish userInfo:userInfo];
        }];
    } else {
        return;
    }
...
- (void)recordVideoCanceled {
    ...
    [[ADVideoManager shareManager] stopRecordingVideo:^(NSString *path) {
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            [[ADVideoManager shareManager] cancelRecordingVideoWithFileName:_videoName];
        });
    }];
}
...
- (void)exit {
    ...
    [self routerEventWithName:ADRouterEventVideoRecordExit userInfo:nil];
}
...
- (void)dealloc {
    [self destroyTimer];
    //    [[ADVideoManager shareManager] exit];// 预防直接返回的情况,但是目前还不行
    //    NSLog(@"ADVideoView------dealloc");
}
